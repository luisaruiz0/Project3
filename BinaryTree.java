public class BinaryTree<T> implements BinaryTreeInterface<T>
{
   ...
   public void postorderTraverse() {
	   postorderTraverse(root); // calls postorder traverse method with root node so it traverses whole tree
   }
   ...
   {
	   return root.getHeight_binaryNodeMethod();S
  ...
  private int getNumberOfNodes(BinaryNode<T> node)
   { 
      if (node!=null)
      {
         ...
         {
            leftCounter = getNumberOfNodes(node.getLeftChild());
         }
         if (node.getRightChild()!=null)
         {
            rightCounter = getNumberOfNodes(node.getRightChild());
         }
